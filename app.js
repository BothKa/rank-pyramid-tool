(function (root) {
  ...
  const ZHEN_ZHENG_UNITS = 10;
  const TRUE_CLEAR_UNITS = ZHEN_MING_UNITS + ZHEN_ZHENG_UNITS;
  let cumulativeTrueWan = 0;
  const GATES = [
    { idx: 0, name: "111丼", base: 2.2, col: "#e74c3c", bg: "#fff5f5" },
    { idx: 1, name: "222丼", base: 8.8, col: "#e67e22", bg: "#fff8f0" },
    { idx: 2, name: "333丼", base: 22, col: "#f39c12", bg: "#fffde7" },
    { idx: 3, name: "444丼", base: 88, col: "#27ae60", bg: "#f0fff4" },
    { idx: 4, name: "555丼", base: 220, col: "#16a085", bg: "#e8fffe" },
    { idx: 5, name: "666丼", base: 880, col: "#2980b9", bg: "#eef6ff" },
    ...
    const zhenMingWan = g.base * ZHEN_MING_UNITS;
    const trueWan = g.base * TRUE_CLEAR_UNITS;
  ...
  function primaryGateForWan(wan) {
    if (wan < PRIMARY_GATES[0].base) return null;
    let result = null;
    for (const gate of PRIMARY_GATES) {
      if (wan >= gate.base) result = gate;
      else break;
    }
    return result;
  ...
  function effectiveUnitAllocation(unitCounts) {
    ...
    const overflowWan = Math.max(0, rawWan - effectiveWan);
  ...
  function unitCountsFromAmounts(amounts) {
    const counts = PRIMARY_GATES.map((gate) => ({ gateIdx: gate.idx, count: 0 }));
    ensureAmounts(amounts).forEach((amount) => {
      const gate = primaryGateForWan(toWan(amount.v));
      if (!gate) return;
      const item = counts.find((candidate) => candidate.gateIdx === gate.idx);
      if (item) item.count += 1;
    });

    return counts.map((item) => ({
      gateIdx: item.gateIdx,
      v: item.count > 0 ? String(item.count) : ""
    }));
  }
  ...
  function unitCountsFromWan(wan) {
    ...
    return PRIMARY_GATES.map((gate, index) => {
      ...
      const units = isLastGate
        ? remaining / gate.base
        : Math.min(TRUE_CLEAR_UNITS, remaining / gate.base);
  ...
  function countProgressForUnits(rawUnits) {
    ...
    const cappedUnits = Math.min(TRUE_CLEAR_UNITS, units);
    const zhenMingDone = Math.min(ZHEN_MING_UNITS, cappedUnits);
    const zhenZhengDone = Math.min(ZHEN_ZHENG_UNITS, Math.max(0, cappedUnits - ZHEN_MING_UNITS));
    return {
      units: cappedUnits,
      zhenMing: {
        done: zhenMingDone,
        total: ZHEN_MING_UNITS,
        missing: Math.max(0, ZHEN_MING_UNITS - zhenMingDone)
      },
      zhenZheng: {
        done: zhenZhengDone,
        total: ZHEN_ZHENG_UNITS,
        missing: Math.max(0, ZHEN_ZHENG_UNITS - zhenZhengDone)
      }
    };
  }
  ...
  function calcUnitCascade(unitCounts, members, gates = PRIMARY_GATES) {
    ...
    for (const gate of gates) {
      ...
      const totalUnits = Math.min(TRUE_CLEAR_UNITS, teamUnits + leaderUnits);
      ...
      const zzNeed = Math.max(0, TRUE_CLEAR_UNITS - teamUnits) * gate.base;

      if (teamUnits >= TRUE_CLEAR_UNITS) {
      ...
      if (totalUnits >= TRUE_CLEAR_UNITS) {
      ...
      const target = reachedZm ? "zz" : "zm";
      const targetUnits = reachedZm ? TRUE_CLEAR_UNITS : ZHEN_MING_UNITS;
    ...
    const activeSteps = steps.filter((step) => (step.totalUnits || 0) > EPSILON);
    const last = activeSteps.filter((step) => step.status === "cleared" || step.status === "covered").at(-1) || null;
    const curr = activeSteps.filter((step) => step.status === "at_zm" || step.status === "wip").at(-1) || null;
    return { steps, last, curr, leaderWan, tc };
  }
  ...
  function analyze(state) {
    ...
    const leaderScoreWan = unitAllocation ? unitAllocation.rawWan : cascade.leaderWan;
  ...
  function renderSummary(result) {
    ...
    els.summary.innerHTML = [
      metric("隊長總額", fmt(result.leaderScoreWan ?? result.leaderWan), `有效配置 ${fmt(result.leaderWan)}`),
  ...
  function renderStatusOverview(result) {
    ...
    const scoreNote = Math.abs(scoreWan - (result.leaderWan || 0)) > EPSILON
      ? `有效配置 ${fmt(result.leaderWan || 0)}`
      : "所有單位加總";
  ...
  function leaderFocus(result) {
    if (result.curr) {
      const statusText = {
        ...
        cleared: "真正過關",
        covered: "團隊真正覆蓋"
      }[result.curr.status] || "進行中";
      const countState = countProgressForUnits(result.curr.totalUnits ?? result.curr.units ?? 0);
      return {
        title: `${result.curr.g.name}・${statusText}`,
        sub: result.curr.gap > 0
          ? `還差 ${fmt(result.curr.gap)} 到${result.curr.g.name}${result.curr.target === "zm" ? "真命" : "真正"}`
          : `${result.curr.g.name}${statusText}`,
        badge: "目前丼級",
        value: `${result.curr.g.name}・${statusText}`,
        note: `真命 ${fmtCount(countState.zhenMing.done)}/${ZHEN_MING_UNITS}・真正補修 ${fmtCount(countState.zhenZheng.done)}/${ZHEN_ZHENG_UNITS}`
      };
    }
      ...
      return `
        <article class="gate-step ${state.className}" style="--gate-color: ${gate.col}; --zm: ${state.zmPct}%; --zz: ${state.zzPct}%;">
          ...
          <div class="gate-step-meta">
            ...
            <span>真正補修 ${escapeHtml(fmt(gate.base * ZHEN_ZHENG_UNITS))}</span>
            <span>真正過關 ${escapeHtml(fmt(gate.trueWan))}</span>
          </div>
          ...
          <div class="gate-counts">
            ...
            ${gateCountLine("真正補修", countState.zhenZheng)}
          </div>
  ...
  function gateCountState(result, gate) {
    ...
    if (step.status === "covered" || step.status === "cleared") return countProgressForUnits(TRUE_CLEAR_UNITS);
  ...
  function handleFieldEdit(event) {
    ...
    if (target.classList.contains("amount-input")) {
      ...
      if (amount) {
        ...
        target.setAttribute("aria-invalid", target.value.trim() !== "" && toWan(target.value) === 0 ? "true" : "false");
        if (target.dataset.scope === "leader") {
